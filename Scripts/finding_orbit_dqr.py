...
data = pd.read_html("http://www-apps.iucaa.in/~astrosat/czti_dqr/", header = 0)[0]
...
for orbit_index, row in orbit_data.loc[obs_index:].iterrows():
    ...
    if (trigtime_utc >= row['Date/time start']) and (trigtime_utc <= row['Date/time end']):
        ...
        print('Path {}: '.format(num), row['Folder'][0:-6]+' '+row['Folder'][-5:], '({} - {})'.format(row['Date/time start'], row['Date/time end']))
        num+=1
